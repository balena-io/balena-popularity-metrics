...
import { Application, getSdk } from 'balena-sdk';
...
const featuredPageCache = new InMemoryCache<FeaturedPage>(FEATURED_PAGE_CACHE_TTL);
...
const formatPageContent = (resource: Application): FeaturedPage => {
	...
	const deviceTypes = contract.data?.supportedDeviceTypes ?? [defaultDeviceType];
	...
	const link = "https://hub.balena.io/organizations/" + authorSlug + "/" + resource.is_of__class + "s/" + name;
	const logo = contract.assets.logo.data.url;

	return { author, class: appClass, description, deviceLogos, link, logo, name };
};

const fetchPageContent = async (id: number): Promise<FeaturedPage | undefined> => {
	try {
		const resource = await sdk.models.application.get(id, {
...
export const getSinglePage = async(id: number): Promise<FeaturedPage | undefined> =>  {
	let page = featuredPageCache.get(id.toString());
	if (!page) {
		page = await fetchPageContent(id);
		page && featuredPageCache.put(id.toString(), page);
	}
	return page;
};